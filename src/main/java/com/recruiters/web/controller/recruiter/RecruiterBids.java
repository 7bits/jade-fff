...
public class RecruiterBids {
     ...
     * Forbidden page if this bid does not belong to current recruiter,
     * Internal Server Error page if something is wrong with obtaining data
     * due to technical or any other reasons
     * @throws Exception in very rare circumstances: it should be runtime
     * or servlet Exception to be thrown
     */
    @RequestMapping(value = "/recruiter-bid-withdraw.json", method = RequestMethod.GET)
    public Object[] applicantApply(
        ...
        try {
            User user = userUtils.getCurrentUser(request);
            recruiterService.withdrawBid(bidId, user.getRecruiterId());
        ...
        return jsonService.recruiterWithdrawBid(locale);
    }
